...
interface Article {
  ...
  title: string;
  content: string;
  image: string; // e.g., "/uploads/articles/grekompost.jpg"
  author: string;
  createdAt: string;
  description: string;
  slug: string;
...
export default function ArticleDetail() {
  ...
  const fetchArticleData = async () => {
    try {
      ...
      // Filter: same author, exclude current article
      const filteredRelated = allArticles
        .filter((a) => a.slug !== slug && a.author === data.author)
        .slice(0, 3)
        .map((a) => ({
          ...a,
              ...
              <div className="lg:w-8/12">
                ...
                {relatedArticles.length > 0 && (
                  <section className="mt-10 bg-white rounded-lg shadow-md p-6">
                    <h2 className="text-3xl font-bold text-green-800 mb-6">Related Articles</h2>
