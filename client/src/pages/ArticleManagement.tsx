...
export default function ArticleManagement() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingArticle) {
        await axios.put(`http://localhost:5000/api/articles/${editingArticle._id}`, fd, {
          headers: { Authorization: token!, 'Content-Type': 'multipart/form-data' }
        });
      } else {
        await axios.post('http://localhost:5000/api/articles', fd, {
          headers: { Authorization: token!, 'Content-Type': 'multipart/form-data' }
        });
      }
  ...
  const handleDeleteArticle = async (_id: string) => {
    ...
    try {
      const token = localStorage.getItem('authToken');
      await axios.delete(`http://localhost:5000/api/articles/${_id}`, {
